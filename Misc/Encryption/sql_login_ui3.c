...
/*
  Test code for a login UI. Uses mcrypt and Rijndael encryption for the passwords stored in the database.
  Initialize table first then you can add some names and passwords. This uses a blob to store the byte
  data in the table so if you have used the sql_login_ui1 or sql_login_ui2 program drop and re-initialize
  the table. It doesn't use a global application encryption key. The keys are made from the user passwords.
  That means that the passwords can't be printed out in the same way sql_login_ui2.c does. The admin doesn't
  know the passwords. 

Get the mcrypt dev library
  apt-get install libmcrypt-dev

  gcc -Wall sql_login_ui3.c -o sql_login_ui3 `pkg-config --cflags --libs gtk+-3.0` -lsqlite3 -lmcrypt 
...
static int encrypt_string(void *buffer, char *key, int buffer_len);
static int decrypt_string(void *buffer, char *key, int buffer_len);
//Database functions.
...
int main(int argc, char **argv)
 {
   ...
   GtkWidget *label3=gtk_label_new("Rijndael Encryption");
...
static void run_database_command(GtkWidget *widget, Widgets *w)
 {
   ...
   gchar *key=(char*)malloc((key_len) * sizeof(char));
                ...
                if(strcmp(password, "password")!=0)
                  {
                    ...
                    if(length2<16) strncpy(key, password, length2);
                    else strncpy(key, password, 15);
                    strcpy(buffer, password); 
                    encrypt_string(buffer, key, buffer_len);
                  }
              ...
              case 2: //Update login
                ...
                if(!login_exists)
                ...
                else
                  {
                    if(strcmp(password, "password")!=0)
                      {
                        decrypt_string(buffer, key, buffer_len);
   ...
   if(result==GTK_RESPONSE_OK)
     {
       ...
       guint length2=gtk_entry_get_text_length(GTK_ENTRY(entry2));
       guint length3=gtk_entry_get_text_length(GTK_ENTRY(entry3));
       guint length4=gtk_entry_get_text_length(GTK_ENTRY(entry4));
       gchar *key1=(char*)malloc((key_len) * sizeof(char));
       gchar *key2=(char*)malloc((key_len) * sizeof(char));
       memset(key1, '\0', key_len);
       memset(key2, '\0', key_len);
               
       if(length3<=2||length4<=4)
         {
           ...
           if(strcmp(password, "password")!=0)
             {
                ...
                if(length2<16) strncpy(key1, password, length2);
                else strncpy(key1, password, 15);
                strcpy(buffer1, password); 
                encrypt_string(buffer1, key1, buffer_len1);
                //New password.
                buffer_len2=allocate_buffer_block(&buffer2, length4);
                if(length2<16) strncpy(key2, new_password, length4);
                else strncpy(key2, new_password, 15);
                strcpy(buffer2, new_password); 
                encrypt_string(buffer2, key2, buffer_len2);
                check=update_login_password(user_name, buffer1, new_user_name, buffer2);
              }
           else
              {
                buffer_len2=allocate_buffer_block(&buffer2, length4);
                if(length4<16) strncpy(key2, new_password, length4);
                else strncpy(key2, new_password, 15);
                strcpy(buffer2, new_password); 
                encrypt_string(buffer2, key2, buffer_len2);
...
int encrypt_string(void *buffer, char *key, int buffer_len)
 {
   int i=0;
...
int decrypt_string(void *buffer, char *key, int buffer_len)
 {
   int i=0;
   //MCRYPT td = mcrypt_module_open("twofish", NULL, "cfb", NULL);
   MCRYPT td = mcrypt_module_open("rijndael-128", NULL, "cbc", NULL);
