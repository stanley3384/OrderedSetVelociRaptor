...
static gboolean get_pixbuf(gpointer *data)
  {
    ...
    gint diff=0;
    if(prev_image!=NULL) diff=compare_images(scale, prev_image, 320, 240);
    if(diff>50000) diff=50000;
    gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(data[2]), (gdouble)diff/50000.0);
    if(prev_image!=NULL) g_object_unref(prev_image);
    prev_image=gdk_pixbuf_copy(scale);
...
static gint compare_images(const GdkPixbuf *current_image, const GdkPixbuf *prev_image, const gint width, const gint height)
  {
    //Warning, Both images are assumed to be the same size.
    gint i=0;
    gint j=0;
    gint step=gdk_pixbuf_get_rowstride(current_image);
    gint channels=gdk_pixbuf_get_n_channels(current_image);
    guchar *pixels1, *pixels2;
    guint length=0;
    pixels1=gdk_pixbuf_get_pixels_with_length(current_image, &length);
    pixels2=gdk_pixbuf_get_pixels_with_length(prev_image, &length);
    gint count1=0;
    gint count2=0;
    gint diff=0;
    gint counter=0;
