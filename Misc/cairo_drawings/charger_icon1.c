...
/*
    Draw a charger icon with cairo. Try a battery and speeding electron look.
...
int main(int argc, char *argv[])
  {
    ...
    gtk_combo_box_text_insert(GTK_COMBO_BOX_TEXT(combo), 0, "1", "Draw Charger Icon1");
    gtk_combo_box_text_insert(GTK_COMBO_BOX_TEXT(combo), 1, "2", "Draw Charger Icon2");
...
static void draw_charger(GtkWidget *da, cairo_t *cr)
  {
    ...
    else draw_gradient2(cr, w1, h1, scale_line);
...
static void draw_gradient2(cairo_t *cr, gdouble w1, gdouble h1, gdouble scale_line)
  {
    ...
    cairo_arc(cr, 6.2*w1, 5.0*h1, 15*scale_line, 0, 2*G_PI);
    cairo_fill(cr);
    cairo_restore(cr);
