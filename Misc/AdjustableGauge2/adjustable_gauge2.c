...
static void adjustable_voltage_gauge_moving(GtkWidget *da, cairo_t *cr, gdouble scale_text, gdouble w1);
...
static void adjustable_speedometer_gauge_moving(GtkWidget *da, cairo_t *cr, gdouble scale_text, gdouble w1);
...
{ 
  ...
  //Internal. No accessor function for these.
  g_object_class_install_property(gobject_class, RESIZE, g_param_spec_boolean("resize", "resize", "resize", FALSE, G_PARAM_READWRITE));

  g_object_class_install_property(gobject_class, PREV_WIDTH, g_param_spec_int("prev_width", "prev_width", "prev_width", 0, 1, 0, G_PARAM_READWRITE));

  g_object_class_install_property(gobject_class, PREV_HEIGHT, g_param_spec_int("prev_height", "prev_height", "prev_height", 0, 1, 0, G_PARAM_READWRITE));

  
  g_object_class_install_property(gobject_class, THREAD, g_param_spec_pointer("thread", "thread", "thread", G_PARAM_READWRITE));

  g_object_class_install_property(gobject_class, SURFACE, g_param_spec_pointer("surface", "surface", "surface", G_PARAM_READWRITE));

  g_object_class_install_property(gobject_class, THREAD_ACTIVE, g_param_spec_boolean("thread_active", "thread_active", "thread_active", FALSE, G_PARAM_READWRITE));
...
static void adjustable_voltage_gauge_draw(GtkWidget *da, cairo_t *cr)
{
  ...
  priv->prev_width=width;
  priv->prev_height=height;

  //Scale.
  if(width<height)
    {
      scale_text=(gdouble)width/400.0;
      w1=(gdouble)width/10.0;
    }
  else
    {
      scale_text=(gdouble)height/400.0;
...
static gpointer thread_draw_new_surface(GtkWidget *da)
{
  ...
  if(priv->gauge_drawing_name==0)
    ...
    {
      cairo_translate(cr, (gdouble)width/2.0, (gdouble)height/2.0);
      //Constant gauge background if window isn't resized or properties aren't changed.
      adjustable_speedometer_gauge_background(da, cr, scale_text, w1);
    } 
  
  cairo_destroy(cr);
  g_idle_add((GSourceFunc)thread_join, da);
  return NULL;
}
static gboolean thread_join(GtkWidget *da)
{  
...
{ 
  ...
  if(priv->surface!=NULL) cairo_surface_destroy(priv->surface);
  if(priv->thread!=NULL) g_thread_join(priv->thread); 
